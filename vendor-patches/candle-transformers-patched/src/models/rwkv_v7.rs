...
impl Model {
    ...
    /// Ouro-style latent re-feed for a single token: run the block stack `n_loops`
    /// times over the *refined* hidden vector, reading logits after each loop.
    ///
    /// Unlike `forward`, which consumes a token once and advances the recurrent
    /// state once, this lets the model "think" about the same token position
    /// multiple times. Loop 1 is the normal pass (embed -> blocks), which commits
    /// the real recurrent-state advance. For loops 2..n we snapshot the per-layer
    /// WKV state, re-run the block stack on the hidden vector produced by the
    /// previous loop, read its logits, then restore the snapshot — so the extra
    /// refinement passes do not permanently corrupt the recurrent state. The
    /// hidden vector itself carries forward between loops (the latent re-feed).
    ///
    /// Returns one logits tensor per loop (length == `n_loops.max(1)`), enabling
    /// confidence-weighted training (Ouro loop-during-training) and adaptive
    /// early-exit inference (DeepConf). With `n_loops == 1` the single returned
    /// logits tensor and the state mutation are identical to `forward`.
    pub fn forward_looped(
        ...
        // --- Loop 1: the real pass that commits the recurrent-state advance. ---
        ...
        let mut v_first: Option<Tensor> = None;
        for block in &self.blocks {
            let (new_xs, new_v_first) = block.forward(&hidden, state, v_first, token_ids_opt)?;
            hidden = new_xs;
            v_first = Some(new_v_first);
        }
        if let Some(dea_state) = &mut state.dea {
        ...
        let logits_of = |h: &Tensor| -> Result<Tensor> {
            let normed = layer_norm(h, &self.ln_out_weight, &self.ln_out_bias, 1e-5)?;
            Ok(normed.unsqueeze(0)?.matmul(&self.head_t)?.squeeze(0)?)
        };

        let mut out = Vec::with_capacity(n);
        out.push(logits_of(&hidden)?);

        // --- Loops 2..n: refine the hidden vector without committing extra
        // recurrent-state advances. Snapshot per-layer WKV, re-run blocks, read
        // logits, then restore so only loop 1's advance persists. ---
        for _ in 1..n {
            let snapshot: Vec<(Tensor, Tensor, Tensor)> = state
                .per_layer
                .iter()
            ...
            // Restore: the refinement passes must not advance the real state.
            for (l, (axp, kv, fxp)) in state.per_layer.iter_mut().zip(snapshot.into_iter()) {
                l.att_x_prev = axp;
                l.att_kv = kv;
                l.ffn_x_prev = fxp;
            }
